    ...
    )
{
  ...
  //struct timeval curr_time, last_time, start_time;
//start of one timestep
for(unsigned iter=0; iter<10; iter++){
  ...
  CUERR;
  //gettimeofday(&curr_time, NULL);
  //fprintf(stderr, "%d : %ld\n", iter, curr_time.tv_sec * 1000000 + curr_time.tv_usec - last_time.tv_sec * 1000000 - last_time.tv_usec);
  //last_time = curr_time;
  ...
  for (zRegionIndex = 0;  zRegionIndex < zRegionDim;  zRegionIndex++) {
    ...
    /*if(zRegionIndex==25){
	    cudaDeviceReset();
	    CUERR;
	    fprintf(stderr, "DeviceReset %d : %ld\n", iter, curr_time.tv_sec * 1000000 + curr_time.tv_usec - last_time.tv_sec * 1000000 - last_time.tv_usec);
	    last_time = curr_time;
    }*/
  ...
  //fprintf(stderr, "%d : %ld\n", iter, curr_time.tv_sec * 1000000 + curr_time.tv_usec - last_time.tv_sec * 1000000 - last_time.tv_usec);
  //last_time = curr_time;
  //fprintf(stderr, "%d : %ld\n", iter, curr_time.tv_sec * 1000000 + curr_time.tv_usec - start_time.tv_sec * 1000000 - start_time.tv_usec);
}
  //gettimeofday(&curr_time, NULL);
  //fprintf(stderr, "%ld\n", curr_time.tv_sec * 1000000 + curr_time.tv_usec);
//End of one timestep
  cudaStreamDestroy(cutoffstream);
  fprintf(stderr, "Finished CUDA kernel calls                        \n");
...
#ifdef NEIGHBOR_COUNT
  fprintf(stderr, "Neighbor count: %f\n", (float)neighbor_count);
#endif
